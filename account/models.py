...
class UserManager(BaseUserManager):
    def create_user(
            self, email, first_name, last_name, iris_roll, 
        ...
        if not iris_roll:
            raise ValueError(_('Users must have a IRIS Roll number'))
        if not is_student:
    ...
    def create_superuser(self, email, first_name, last_name, iris_roll,
                         password, is_student, is_instructor):
        """
        Creates and saves a superuser with the given email, first name,
        last name, iris_roll, and password.
        """
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    iris_roll = models.CharField(max_length = 8, unique=True)
    ...
    REQUIRED_FIELDS = ['first_name', 'last_name', 'iris_roll', 'is_student', 'is_instructor']
    ...
    def get_iris_roll(self):
        """
        Return the iris_roll.
        """
        e_id = self.iris_roll
        return e_id
