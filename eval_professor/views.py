...
def make_new_course(request):
    ...
    co_prof_valid = False
    if co_professor_id != '':
        co_professor = User.objects.filter(iris_roll=co_professor_id).first()
        if co_professor is None:
            ...
            return redirect('create-new-course')
        elif co_professor_id == request.user.iris_roll:
...
def add_student(request, course_id):
    ...
    if iris_roll != '':
        new_student = User.objects.filter(iris_roll = iris_roll).first()
        valid_student = True
    elif email != '':
...
def remove_student(request, course_id, iris_roll):
    course = Course.objects.get(pk=course_id)
    student = User.objects.get(iris_roll=iris_roll)
...
def add_student_to_team(request, team_id):
    ...
    if valid_student:
        ...
        for each_team in team_list:
            if len(each_team.student.filter(iris_roll=iris_roll)) > 0:
...
def remove_student_from_team(request, team_id, iris_roll):
...
def download_csv(request, assessment_id):
    ...
    for result_set in assessment.result_sets.all():
        ...
        row.append(result_set.student.iris_roll)
